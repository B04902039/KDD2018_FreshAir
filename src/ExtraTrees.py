...
drop = ['utc_time' , 'year', 'quarter', 'month', 'week', 'weekday', 'date', 'day', 'hour', 'area']
...
for s in stationId:
  ...
  X_train = X[:-48]
  y_train = np.zeros((X_train.shape[0], 48 * 3))
  for i in range(y_train.shape[0]):
    for j in range(24):
      y_train[i, 3 * j] = X[i + 24, train.shape[1] * j]
      y_train[i, 3 * j + 1] = X[i + 24, train.shape[1] * j + 1]
      y_train[i, 3 * j + 2] = X[i + 24, train.shape[1] * j + 4]
      y_train[i, 3 * j + 72] = X[i + 48, train.shape[1] * j]
      y_train[i, 3 * j + 72 + 1] = X[i + 48, train.shape[1] * j + 1]
      y_train[i, 3 * j + 72 + 2] = X[i + 48, train.shape[1] * j + 4]
  """
  X_val = X_[:-48]
  y_val = np.zeros((X_val.shape[0], 48 * 3))
  for i in range(y_val.shape[0]):
    for j in range(24):
      y_val[i, 3 * j] = X_[i + 24, val.shape[1] * j]
      y_val[i, 3 * j + 1] = X_[i + 24, val.shape[1] * j + 1]
      y_val[i, 3 * j + 2] = X_[i + 24, val.shape[1] * j + 4]
      y_val[i, 3 * j + 72] = X_[i + 48, val.shape[1] * j]
      y_val[i, 3 * j + 72 + 1] = X_[i + 48, val.shape[1] * j + 1]
      y_val[i, 3 * j + 72 + 2] = X_[i + 48, val.shape[1] * j + 4]
      #y_val[i, j + 24] = X_[i + 48, val.shape[1] * j]
  """

  X_test = X_[:-48]
  y_test = np.zeros((X_test.shape[0], 48 * 3))
  for i in range(y_test.shape[0]):
    for j in range(24):
      y_test[i, 3 * j] = X_[i + 24, test.shape[1] * j]
      y_test[i, 3 * j + 1] = X_[i + 24, test.shape[1] * j + 1]
      y_test[i, 3 * j + 2] = X_[i + 24, test.shape[1] * j + 4]
      y_test[i, 3 * j + 72] = X_[i + 48, test.shape[1] * j]
      y_test[i, 3 * j + 72 + 1] = X_[i + 48, test.shape[1] * j + 1]
  ...
  #  print("(%f) %s %d" % (importances[indices[f]] / importances[indices[0]], IDX[indices[f] % 14], 24 - indices[f] // 14))
  predictions = regressor.predict(X_test)
  print(smape(y_test, predictions))
  SMAPE.append(float(smape(y_test, predictions)))
  print(smape(y_test[:, :72], predictions[:, :72]))
  SMAPE24.append(float(smape(y_test[:, :72], predictions[:, :72])))
  print(smape(y_test[:, 72:], predictions[:, 72:]))
  SMAPE48.append(float(smape(y_test[:, 72:], predictions[:, 72:])))
